...
WHATSAPP_MANAGER_NATIVE_ADAPTER_ENABLED_VALUE="${WHATSAPP_MANAGER_NATIVE_ADAPTER_ENABLED:-auto}"
WHATSAPP_MANAGER_API_URL_VALUE="${WHATSAPP_MANAGER_API_URL:-http://127.0.0.1:3000}"
WHATSAPP_MANAGER_API_TOKEN_VALUE="${WHATSAPP_MANAGER_API_TOKEN:-${API_TOKEN:-local-dev-token}}"
WHATSAPP_MANAGER_ALLOW_ALL_USERS_VALUE="${WHATSAPP_MANAGER_ALLOW_ALL_USERS:-true}"
WHATSAPP_MANAGER_ALLOWED_USERS_VALUE="${WHATSAPP_MANAGER_ALLOWED_USERS:-}"
WHATSAPP_MANAGER_HOME_CHANNEL_VALUE="${WHATSAPP_MANAGER_HOME_CHANNEL:-}"
WHATSAPP_MANAGER_POLL_INTERVAL_VALUE="${WHATSAPP_MANAGER_POLL_INTERVAL:-1}"
WHATSAPP_MANAGER_PAGE_SIZE_VALUE="${WHATSAPP_MANAGER_PAGE_SIZE:-50}"
...
export WHATSAPP_MANAGER_NATIVE_ADAPTER_ENABLED="${WHATSAPP_MANAGER_NATIVE_ADAPTER_ENABLED_VALUE}"
export WHATSAPP_MANAGER_API_URL="${WHATSAPP_MANAGER_API_URL_VALUE}"
export WHATSAPP_MANAGER_API_TOKEN="${WHATSAPP_MANAGER_API_TOKEN_VALUE}"
export WHATSAPP_MANAGER_ALLOW_ALL_USERS="${WHATSAPP_MANAGER_ALLOW_ALL_USERS_VALUE}"
export WHATSAPP_MANAGER_ALLOWED_USERS="${WHATSAPP_MANAGER_ALLOWED_USERS_VALUE}"
export WHATSAPP_MANAGER_HOME_CHANNEL="${WHATSAPP_MANAGER_HOME_CHANNEL_VALUE}"
export WHATSAPP_MANAGER_POLL_INTERVAL="${WHATSAPP_MANAGER_POLL_INTERVAL_VALUE}"
export WHATSAPP_MANAGER_PAGE_SIZE="${WHATSAPP_MANAGER_PAGE_SIZE_VALUE}"
...
printf 'export HERMES_API_KEY=%q\nexport API_SERVER_KEY=%q\nexport WHATSAPP_MANAGER_NATIVE_ADAPTER_ENABLED=%q\nexport WHATSAPP_MANAGER_API_URL=%q\nexport WHATSAPP_MANAGER_API_TOKEN=%q\nexport WHATSAPP_MANAGER_ALLOW_ALL_USERS=%q\nexport WHATSAPP_MANAGER_ALLOWED_USERS=%q\nexport WHATSAPP_MANAGER_HOME_CHANNEL=%q\nexport WHATSAPP_MANAGER_POLL_INTERVAL=%q\nexport WHATSAPP_MANAGER_PAGE_SIZE=%q\n' \
  "${INTERNAL_API_KEY_VALUE}" \
  "${INTERNAL_API_KEY_VALUE}" \
  "${WHATSAPP_MANAGER_NATIVE_ADAPTER_ENABLED_VALUE}" \
  "${WHATSAPP_MANAGER_API_URL_VALUE}" \
  "${WHATSAPP_MANAGER_API_TOKEN_VALUE}" \
  "${WHATSAPP_MANAGER_ALLOW_ALL_USERS_VALUE}" \
  "${WHATSAPP_MANAGER_ALLOWED_USERS_VALUE}" \
  "${WHATSAPP_MANAGER_HOME_CHANNEL_VALUE}" \
  "${WHATSAPP_MANAGER_POLL_INTERVAL_VALUE}" \
  "${WHATSAPP_MANAGER_PAGE_SIZE_VALUE}" \
  >> /opt/data/.bashrc
printf 'export HERMES_API_KEY=%q\nexport API_SERVER_KEY=%q\nexport WHATSAPP_MANAGER_NATIVE_ADAPTER_ENABLED=%q\nexport WHATSAPP_MANAGER_API_URL=%q\nexport WHATSAPP_MANAGER_API_TOKEN=%q\nexport WHATSAPP_MANAGER_ALLOW_ALL_USERS=%q\nexport WHATSAPP_MANAGER_ALLOWED_USERS=%q\nexport WHATSAPP_MANAGER_HOME_CHANNEL=%q\nexport WHATSAPP_MANAGER_POLL_INTERVAL=%q\nexport WHATSAPP_MANAGER_PAGE_SIZE=%q\n' \
  "${INTERNAL_API_KEY_VALUE}" \
  "${INTERNAL_API_KEY_VALUE}" \
...
if [ "$#" -gt 0 ] \
  && [ "$1" = "auto-bot-services" ] \
  && ! sudo -E -H -u hermes test -r /opt/app/package.json; then
  echo "Source mount is not readable by hermes; running app services as root." >&2
  exec "$@"
fi
